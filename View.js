...
function fieldSet(labelText, inputValue, oninput) {
  return div([
    ...
    input({
      ...
      type: 'text',
      value: inputValue,
      oninput// oninput function is the e => dispatch etc.
    })
  ]);
};
...
function buttonSet(dispatch) {
  return div([
    button(
      {
        className: 'f3 pv2 ph bg-blue white bn mr2 dim',
        type: 'submit',
      },
      'Save',
    ),
...
function view(dispatch, model) {
  return div({ className: 'mw6 center' },[
    ...
    pre(JSON.stringify(model, null, 2)),//render object literal model as string on app.
  ]);
};
//tableView(dispatch, model.meals)
